...
export function searchReleases() {
    ...
    axios.get(`http://localhost:8080/backend-1.0-SNAPSHOT/soundkraut/search/query?title=${title}&artist=${artist}&genre=${genre}`)
        .then(response => releaseSearchResult.update(old => response.data))
}

export function searchDetails(releaseId) {
    axios.get(`http://localhost:8080/backend-1.0-SNAPSHOT/soundkraut/search/id/${releaseId}`)
